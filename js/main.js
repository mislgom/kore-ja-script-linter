 ...
 * main.js v4.51 - Vertex AI API 키 + Gemini 2.5 Flash
...
console.log('🚀 main.js v4.51 로드됨');
console.log('📌 v4.51: 1차/2차 분석 프롬프트 강화 (오류 검출 정확도 향상)');
...
function initApp() {
    ...
    console.log('✅ main.js v4.51 초기화 완료');
}
...
function buildStage1Prompt(script) {
    ...
    return '당신은 조선시대 사극 대본 전문 검수자입니다. 반드시 오류를 찾아내야 합니다.\n\n' +
        ...
        '- 신분에 맞지 않는 말투 (양반이 하인처럼 말하거나 그 반대)\n' +
        '- 성격 불일치 (앞에서 소심했던 인물이 갑자기 대담해지는 등)\n' +
        '- 인물 이름 오타나 혼동\n\n' +
        '### 3. 호칭 오류 (필수 검사)\n' +
        '- 신분에 맞지 않는 호칭\n' +
        '- 관계에 맞지 않는 호칭 (아버지를 형님이라 부르는 등)\n' +
        '- 같은 인물을 다른 호칭으로 부르는 경우\n\n' +
        '### 4. 이야기 흐름 오류 (필수 검사)\n' +
        '- 시간 순서 오류 (밤인데 해가 떠있다는 표현 등)\n' +
        '- 장소 이동 오류 (한양에 있다가 갑자기 부산에 있는 등)\n' +
        '- 앞뒤 문맥 불일치\n' +
        '- 죽은 인물이 다시 등장\n\n' +
        '### 5. 대사 자연스러움 (필수 검사)\n' +
        '- 어색한 문장 구조\n' +
        ...
        '- 너무 현대적인 말투\n\n' +
        '## ⚠️ 나레이션 규칙\n' +
        '- "나레이션:", "NA:", "N:" 등으로 시작하는 줄은 나레이션입니다\n' +
        '- 나레이션의 고어체 어미(~하였느니라, ~이니라 등)는 오류가 아닙니다\n' +
        '- 단, 나레이션에서도 시대착오적 단어(컴퓨터, 자동차 등)가 있으면 오류입니다\n\n' +
        '## 📝 분석 대상 대본:\n```\n' + script + '\n```\n\n' +
        '## ✅ 응답 규칙\n' +
        '1. 최소 1개 이상의 오류를 반드시 찾아주세요\n' +
        '2. 오류가 정말 없다면 문장을 더 자연스럽게 다듬을 부분을 찾아주세요\n' +
        ...
        '  {"type": "호칭오류", "original": "원문", "revised": "수정문", "reason": "사유", "severity": "medium"},\n' +
        '  {"type": "이야기흐름", "original": "원문", "revised": "수정문", "reason": "사유", "severity": "low"},\n' +
        '  {"type": "대사자연스러움", "original": "원문", "revised": "수정문", "reason": "사유", "severity": "low"}\n' +
        ']}\n' +
        '```\n\n' +
        '⚠️ 중요: 오류를 하나도 찾지 못했다면, 대본을 다시 읽고 더 꼼꼼히 분석하세요!';
}
...
function buildStage2Prompt(script) {
    return '당신은 조선시대 사극 대본 전문 작가이자 품질 검수 전문가입니다.\n' +
        ...
        '```\n\n' +
        '⚠️ 중요:\n' +
        '1. errors 배열에 최소 1개 이상의 개선점을 넣어주세요\n' +
        '2. 점수는 현실적으로 엄격하게 매겨주세요 (80점대가 좋은 점수입니다)\n' +
        '3. perfectScript에는 전체 대본을 빠짐없이 포함하고, 수정된 부분 앞에 ★를 붙여주세요';
}
