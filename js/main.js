...
[검수 항목]
1) 국가 배경 검증 - 한국 배경에 타국 요소 혼입 여부
2) 시대 배경 검증 - 시대에 맞지 않는 표현 수정
3) 등장인물 설정 일관성 - 이름/나이/외형/성격/말투 통일
4) 등장인물 관계 일관성 - 호칭/관계 통일

[analysis 작성 규칙 - TSV 형식 필수]
반드시 아래 형식으로 작성 (탭으로 구분):
...
오류 발견 시 (예시):
번호\t유형\t위치(대략)\t변경 내용 요약\t검수 포인트
1\t시간 왜곡\t도입부\t겨울 폭설 → 초가을 새벽\t시간 연속성
2\t인물 설정 변경\t중반부\t과부 → 30년차 아전\t캐릭터 일관성

[출력 형식 - JSON만]
{"analysis":"(TSV 형식)","revised":"(수정된 대본)"}
...
[검수 항목]
1) 장소 왜곡 - 장면 연속성 점검
2) 시간 왜곡 - 시간 흐름 논리성 점검
3) 등장인물 설정 변경 - 성격/직업/관계 일관성
4) 쌩뚱맞는 상황 - 복선 없는 사건 점검
5) 대화 흐름 붕괴 - 질문-답변 연결성
6) 쌩뚱 인물 등장 - 소개 없는 인물 점검
...
오류 발견 시:
번호\t유형\t위치(대략)\t변경 내용 요약\t검수 포인트
1\t시간 왜곡\t도입부\t아침 → 저녁 급변\t시간 연속성
2\t대화 붕괴\t나그네 대화\t숙박 요청 → 쌀값 화제\t대사 논리

[출력 규칙]
- 오류 없음: {"analysis":"(TSV)","revised":"(최종 대본)"}
- 오류 발견: {"analysis":"(TSV)","revised":""}
