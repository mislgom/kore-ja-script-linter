...
function buildRolePrompt(roleId, chunkText, chunkInfo, scriptLength) {
    ...
    var factCheckGuide = '\n\n## 🔍 팩트체크 지침 (너의 학습 데이터 기반)\n' +
        '당신은 2024~2025년까지의 경제 데이터를 학습하고 있습니다.\n' +
        '대본에 등장하는 경제 수치가 실제 데이터와 다른지 반드시 팩트체크하세요.\n\n' +
        '### 팩트체크 대상:\n' +
        '- 각국 중앙은행 기준금리 (한국은행, 연준, ECB, BOJ 등)\n' +
        '- GDP 성장률 (한국, 미국, 중국, 유로존 등)\n' +
        '- 소비자물가지수(CPI), 근원 인플레이션\n' +
        '- 실업률, 비농업고용(NFP)\n' +
        '- 환율 (원/달러, 달러/엔, 유로/달러 등)\n' +
        '- 주요 주가지수 (코스피, S&P500, 나스닥, 다우존스 등)\n' +
        '- 국채 수익률 (미국 10년물, 한국 3년물 등)\n' +
        '- FOMC/금통위 회의 일정 및 결정 사항\n' +
        '- IMF/OECD 경제전망 수치\n' +
        '- 유가 (WTI, 브렌트)\n' +
        '- 금값, 비트코인 등 주요 자산 가격 추이\n\n' +
        '### 팩트체크 방법:\n' +
        '1. 대본에 "2024년 미국 기준금리 3.5%"라고 적혀있으면 → 너의 학습 데이터로 실제 수치를 확인\n' +
        '2. 실제와 다르면 → 오류로 보고하고 revised에 올바른 수치를 제안\n' +
        '3. 확실히 아는 수치만 팩트체크하세요. 불확실하면 보고하지 마세요\n' +
        '4. 대본의 시점과 너의 데이터 시점이 일치하는지 반드시 확인하세요\n\n' +
        '### 팩트체크 오류 보고 시:\n' +
        '- type은 "팩트체크"로 표기\n' +
        '- reason에 "실제: [올바른 수치]" 형태로 근거를 명시\n' +
        '- severity는 수치 차이가 클수록 "high"\n';

    var rulesString = getEconomyRulesString();

    if (roleId === 'role1_numbers') {
        return header +
            '## 🎯 당신의 역할: 수치·단위·데이터 검증관 + 팩트체커\n\n' +
            dataValidationGuide + '\n' +
            factCheckGuide + '\n' +
            ...
            '### 검사항목 4: 팩트체크 (분석 강도: 100%)\n' +
            '대본에 등장하는 모든 경제 수치를 너의 학습 데이터와 대조하세요.\n' +
            '실제 데이터와 다른 수치가 있으면 오류로 보고하세요.\n' +
            '확실히 아는 것만 보고하고, 불확실하면 보고하지 마세요.\n\n' +
            'type은 "팩트체크"로 표기하세요.\n' +
            footer;
    }
    ...
    if (roleId === 'role2_logic') {
        return header +
            ...
            'type은 "공포조장"으로 표기하세요.\n\n' +
            '### 검사항목 4: 경제 논리 팩트체크 (분석 강도: 90%)\n' +
            '대본에서 경제 인과관계를 주장할 때, 그 주장이 실제 경제 이론/역사와 맞는지 확인하세요.\n' +
            '예: "금리를 인상하면 주가가 반드시 오른다" → 일반적으로 반대 (오류)\n' +
            '예: "달러가 강세면 원자재 가격이 오른다" → 일반적으로 반대 (오류)\n' +
            '확실히 틀린 경제 논리만 보고하세요.\n\n' +
            'type은 "팩트체크"로 표기하세요.\n' +
            footer;
    }
    ...
    if (roleId === 'role3_risk') {
        return header +
            ...
            'type은 "출처누락"으로 표기하세요.\n\n' +
            '### 검사항목 4: 정책·이벤트 팩트체크 (분석 강도: 100%)\n' +
            '대본에 언급된 정책 결정, 경제 이벤트가 실제로 있었는지 확인하세요.\n' +
            '예: "2024년 3월 FOMC에서 금리를 인하했다" → 실제 인하 여부 확인\n' +
            '예: "한국은행이 2024년 1월에 기준금리를 인상했다" → 실제 결정 확인\n' +
            '존재하지 않는 정책, 허위 회의 일정, 가짜 결정을 보고하세요.\n' +
            '확실히 아는 것만 보고하고, 불확실하면 보고하지 마세요.\n\n' +
            'type은 "팩트체크"로 표기하세요.\n' +
            footer;
    }
